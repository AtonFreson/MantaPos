...
def iter_function_no_itertools(a, b, c, d):
    """
    Calculates the next permutation of (0, 1, 2, 3) in lexicographical order
    without using the itertools library.

    The inputs a, b, c, d must be unique integers from the set {0, 1, 2, 3}.

    Args:
      a: The first element (integer 0-3).
      b: The second element (integer 0-3).
      c: The third element (integer 0-3).
      d: The fourth element (integer 0-3).

    Returns:
      A tuple representing the next permutation (a', b', c', d').
      Wraps around from the last permutation (3, 2, 1, 0) to the first (0, 1, 2, 3).

    Raises:
      ValueError: If the input (a, b, c, d) is not a valid permutation of (0, 1, 2, 3)
                  (i.e., values are not unique or not in the range 0-3).
    """
    perm_list = [a, b, c, d]

    # --- Input Validation ---
    if len(perm_list) != 4 or set(perm_list) != {0, 1, 2, 3}:
        raise ValueError("Input (a, b, c, d) must be a permutation of (0, 1, 2, 3)")

    n = 4 # Length of the permutation

    # --- Find Next Permutation Algorithm ---
    # Step 1: Find the largest index k such that perm_list[k] < perm_list[k + 1]
    # Iterate from right-to-left, starting at the second-to-last element.
    k = -1
    ...
    # Step 1b: If no such index exists, the permutation is the last one (e.g., [3, 2, 1, 0]).
    # The next permutation is the first one (sorted: [0, 1, 2, 3]).
    if k == -1:
        perm_list.sort() # Sort in place to get the first permutation
        return tuple(perm_list)

    # Step 2: Find the largest index l > k such that perm_list[k] < perm_list[l]
    # Iterate from right-to-left, starting from the last element.
    l = -1 # Should always find an 'l' if 'k' was found
    for i in range(n - 1, k, -1):
        if perm_list[k] < perm_list[i]:
            l = i
            break

    # Step 3: Swap the value of perm_list[k] with that of perm_list[l]
    perm_list[k], perm_list[l] = perm_list[l], perm_list[k]

    # Step 4: Reverse the sequence from index k + 1 up to and including the final element.
    # Use two pointers, one starting at k + 1 and one at the end.
    left = k + 1
    right = n - 1
    while left < right:
    ...
    # Return the new permutation as a tuple
    return tuple(perm_list)
...
while True:
    ...
    quad_marker_rot = [[0,0,90], [t,p,s], [0,0,0], [0,0,90]]
